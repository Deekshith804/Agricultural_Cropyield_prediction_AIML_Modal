...
# Global variable to store feature importance data
...
app.layout = dbc.Container([
    ...
    dbc.Tabs([
        ...
        dbc.Tab([
            dbc.Row([
                dbc.Col([
                    html.H3("Data Analysis & Insights", className="mb-4"),
                    html.P("Explore agricultural data patterns and trends.")
                ])
            ]),
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H5("Temperature vs Yield Relationship", className="card-title"),
                            dcc.Graph(id="temp-yield-chart")
                        ])
                    ])
                ], width=6),
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H5("Rainfall vs Yield Relationship", className="card-title"),
                            dcc.Graph(id="rainfall-yield-chart")
                        ])
                    ])
                ], width=6)
            ], className="mb-4"),
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H5("Soil pH Distribution", className="card-title"),
                            dcc.Graph(id="ph-distribution-chart")
                        ])
                    ])
                ], width=6),
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H5("Nutrient Levels", className="card-title"),
                            dcc.Graph(id="nutrient-chart")
                        ])
                    ])
                ], width=6)
            ])
        ], label="📊 Data Analysis", tab_id="analysis"),
...
def predict_yield(n_clicks, temp, rainfall, humidity, ph, nitrogen, 
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Store feature importance data for the chart
            global feature_importance_data
            feature_importance_data = result.get('feature_importance', {})
...
def update_feature_importance(n_clicks):
    ...
    try:
        # Use actual feature importance data from the global variable
        global feature_importance_data
...
@app.callback(
    Output("temp-yield-chart", "figure"),
    Output("rainfall-yield-chart", "figure"),
    Output("ph-distribution-chart", "figure"),
...
def update_charts(active_tab):
    ...
    temp_fig = px.scatter(df, x='temperature', y='yield', 
                          title="Temperature vs Crop Yield",
                          labels={'temperature': 'Temperature (°C)', 'yield': 'Yield (tons/hectare)'})
    
    # Rainfall vs Yield
    rain_fig = px.scatter(df, x='rainfall', y='yield',
                          title="Rainfall vs Crop Yield",
                          labels={'rainfall': 'Rainfall (mm/month)', 'yield': 'Yield (tons/hectare)'})
    
    # pH Distribution
    ph_fig = px.histogram(df, x='soil_ph', nbins=20,
                          title="Soil pH Distribution",
                          labels={'soil_ph': 'Soil pH', 'count': 'Frequency'})
    
    # Nutrient Levels
    nutrient_fig = go.Figure()
    nutrient_fig.add_trace(go.Bar(name='Nitrogen', x=['Nitrogen'], y=[df['nitrogen'].mean()]))
    nutrient_fig.add_trace(go.Bar(name='Phosphorus', x=['Phosphorus'], y=[df['phosphorus'].mean()]))
    nutrient_fig.update_layout(title="Average Nutrient Levels", barmode='group')
    
    return temp_fig, rain_fig, ph_fig, nutrient_fig
